...
const VideoList = ({ videos }) => {
    ...
    if (!videos.length) {
        return <div className="text-black text-center pt-12 text-2xl">Busca tus películas y series favoritas...</div>;
    }

    return (
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {videos.map(video => (
                <div key={video.id.videoId} className="bg-gray-900 rounded-lg overflow-hidden transform hover:scale-105 transition-transform duration-300">
                    <a href={`https://www.youtube.com/watch?v=${video.id.videoId}`} target="_blank" rel="noopener noreferrer">
                        <img src={video.snippet.thumbnails.medium.url} alt={video.snippet.title} className="w-full" />
                        <div className="p-4">
                            <h3 className="text-white font-bold truncate">{video.snippet.title}</h3>
...
function Navbar() {
    ...
    const handleSearch = async (e) => {
        ...
        setError(null);
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query.trim()}&key=${API_KEY}&maxResults=20&type=video`);
            if (!response.ok) {
    ...
    return (
        ...
        <nav className="m-2 p-2 flex justify-between items-center bg-black text-white rounded-3xl fixed top-0 inset-x-0 z-40">
            ...
            <div className="flex items-center space-x-5">
                {isSearchVisible ? (
                    <form onSubmit={handleSearch} className="flex items-center bg-gray-800 rounded-md">
                        <input
                            type="text"
                            placeholder="Buscar..."
                        ...
                        />
                        <button type="submit" aria-label="Buscar" className="p-2 hover:text-gray-300">
                            <FaSearch size={20} />
                        </button>
                        <button type="button" onClick={toggleSearch} aria-label="Cerrar búsqueda" className="p-2 hover:text-gray-300">
                            <FaTimes size={20} />
                        </button>
                    </form>
                ) : (
        ...
        <main className="pt-24"> {/* Padding superior para que el contenido no quede debajo de la nav */}
            {isLoading && <div className="text-black text-center pt-12 text-2xl">Buscando...</div>}
            {error && !isLoading && <div className="text-red-500 text-center pt-12 text-2xl">{error}</div>}
